...
public class BloomFilter {
    ...
    private final ArrayList<String> hashFunctionsSaltList = new ArrayList<>();
    ...
    public BloomFilter(long expectedInsertions, double falsePositiveProbability, boolean isLogging) {
...
//            log.error("bitmap信息保存失败:{}", e.getMessage());
    ...
    public boolean generatorInfo() {
        BufferedWriter writerInfo = null;
        try {
            writerInfo = new BufferedWriter(new FileWriter("information/info.txt" ,true));
...
//            log.error("生成信息文件失败:{}", e.getMessage());
            logIfEnabled(log::info, "生成信息文件失败: " + e.getMessage());
            return false;
        }finally {
//            log.info("过滤器信息保存成功");
            logIfEnabled(log::info, "过滤器信息保存成功");
            try {
                writerInfo.close();
            } catch (IOException e) {
